  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-20 bg-[#0d0d0d] rounded-xl border border-white/5 animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <Card className="p-10 flex flex-col items-center justify-center text-center">
        ...
        <h4 className="text-sm font-semibold text-[#f5f5f5]">Sin recurrentes configurados</h4>
          ...
          >
            ...
            <div className="flex items-center gap-3 text-left">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="text-sm font-semibold text-[#f5f5f5]">{item.name}</h4>
                ...
                <div className="flex items-center gap-2 text-xs text-[#8a8a8a] mt-0.5">
                  ...
                  <span>•</span>
                  <span>Próximo: {formatDate(item.next_date)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between md:justify-end w-full md:w-auto gap-4">
              <div className="text-left md:text-right">
                ...
                <span className="text-[10px] text-[#666] block uppercase tracking-wider">
                  {item.type.replace('_', ' ')}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`w-9 h-5 rounded-full transition-colors cursor-pointer relative p-0.5 ${
                    item.is_active ? 'bg-[#FE9D01]' : 'bg-white/20'
                  }`}
