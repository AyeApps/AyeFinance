...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onBack }) => {
  ...
  const [googleRequest, googleResponse, promptGoogleAsync] = Google.useIdTokenAuthRequest({
    ...
    redirectUri: AuthSession.makeRedirectUri({ scheme: 'ayefinance' }),
  });
    ...
    try {
      ...
      if (Platform.OS === 'ios' && isAppleAuthAvailable) {
        ...
        const statePayload = JSON.stringify({
          origin: (Platform.OS === 'web' && typeof window !== 'undefined' && window.location) 
                    ? window.location.origin 
                    : AuthSession.makeRedirectUri({ scheme: 'ayefinance' }).replace(/\/$/, ''),
          app: 'finance',
          nonce,
        });
        ...
        const nativeReturnUrl = AuthSession.makeRedirectUri({ scheme: 'ayefinance' });
  ...
  return (
    <View style={[styles.appContainer, { backgroundColor: 'transparent', paddingBottom: bottomInset + 16 }]}>
      ...
      {onBack && (
        <View style={[styles.topLeftControls, { top: Math.max(topInset, 16) + 8 }]}>
          <TouchableOpacity
            style={[
          ...
          <View style={[styles.techFrameContent, isMobile && styles.techFrameContentMobile]}>
            ...
            <View style={[styles.titleSection, isMobile && styles.titleSectionMobile]}>
              <View style={[styles.authLogoBox, isMobile && styles.authLogoBoxMobile]}>
                <AyeFinanceLogo size={isMobile ? 40 : 56} />
              </View>
              ...
              >
                {isLoading ? (
                  <View style={styles.btnLoadingRow}>
                    ...
                    <Text style={[styles.heroBtnText, { color: '#000000' }]}>
                      {t.auth.processing}
                    </Text>
                  </View>
                ) : (
