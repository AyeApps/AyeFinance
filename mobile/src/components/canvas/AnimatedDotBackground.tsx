...
const WebInteractiveDots: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // Continuous drifting offset (diagonal movement) - disabled when reduced motion is requested
      const driftX = prefersReducedMotion ? 0 : (now * driftSpeedX) % totalGridW;
      const driftY = prefersReducedMotion ? 0 : (now * driftSpeedY) % totalGridH;
...
export const AnimatedDotBackground: React.FC = () => {
  ...
  useEffect(() => {
    if (Platform.OS === 'web' || reduceMotion || isSidebarOpen) {
      anim.setValue(0);
      return;
    }
    ...
    loop.start();
    return () => loop.stop();
  }, [anim, reduceMotion, isSidebarOpen]);
    ...
    <View pointerEvents="none" style={[StyleSheet.absoluteFill, styles.nativeContainer]}>
      ...
      >
        <Svg width="100%" height="100%">
          ...
          <Rect width="100%" height="100%" fill="url(#ayefinance-dot-grid)" />
        </Svg>
      </Animated.View>
    </View>
  );
