...
export const TransactionForm: React.FC<TransactionFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left">
      ...
      <div className="grid grid-cols-3 gap-2 bg-[var(--bg-secondary)] p-1 rounded-xl border border-[var(--border-muted)]">
        <button
          type="button"
          ...
          className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
            type === 'gasto'
              ? 'bg-[var(--accent-danger)] text-white shadow-[0_2px_10px_rgba(255,23,68,0.35)]'
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
            type === 'ingreso'
              ? 'bg-[var(--accent-success)] text-black shadow-[0_2px_10px_rgba(0,230,118,0.35)]'
              : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface)]'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
            type === 'transferencia'
              ? 'bg-[var(--accent-info)] text-white shadow-[0_2px_10px_rgba(0,176,255,0.35)]'
              : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface)]'
          }`}
        >
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-[var(--text-secondary)] font-mono uppercase font-bold tracking-wider">Monto</label>
        <div className="relative">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xl font-bold font-mono text-[var(--text-muted)]">
            $
          </span>
          <input
            type="number"
            ...
            className="w-full bg-[var(--bg-secondary)] border border-[var(--border-muted)] rounded-xl pl-9 pr-4 py-3 text-2xl font-mono font-extrabold text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-amber)] transition-all placeholder:text-[var(--text-muted)]"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs text-[var(--text-secondary)] font-mono uppercase font-bold tracking-wider">Categoría</label>
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${
                category === cat
                  ...
                  : 'bg-[var(--bg-secondary)] border border-[var(--border-muted)] text-[var(--text-secondary)] hover:border-[var(--accent-amber)] hover:text-[var(--text-primary)]'
              }`}
            >
              {cat}
            </button>
