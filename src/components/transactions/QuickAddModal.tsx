      ...
      if (accs.length > 0) {
        setAccountId((prev) => {
          if (initialData?.accountId && accs.some((a) => a.id === initialData.accountId)) {
            return initialData.accountId;
          }
          return prev && accs.some((a) => a.id === prev) ? prev : accs[0].id;
        });

        if (initialData?.destinationAccountId && accs.some((a) => a.id === initialData.destinationAccountId)) {
          setDestinationAccountId(initialData.destinationAccountId);
        } else {
          setDestinationAccountId((prev) => {
            const currentOrigin = initialData?.accountId || accs[0].id;
            if (prev && accs.some((a) => a.id === prev && a.id !== currentOrigin)) {
              return prev;
            }
            const other = accs.find((a) => a.id !== currentOrigin);
